  ...
  const handleResize = () => {
    ...
    timeout = setTimeout(() => {
      if (window.innerHeight < 500) {
        // Keyboard likely open → push box down
        setSearchPosition((prev) => ({ ...prev, y: 80 }));
      }
      // ✅ Don't reset to default, keep last dragged position when keyboard closes
    }, 150);
  };
...
{/* Accuracy Circle (Blue Transparent) */}
{userLocation && userAccuracy && (
  <Circle
    center={userLocation}
    radius={userAccuracy}
    options={{
      fillColor: "#4da6ff44",   // light blue transparent fill
      strokeColor: "#4da6ff",   // outline
      strokeWeight: 1,
    }}
  />
)}
